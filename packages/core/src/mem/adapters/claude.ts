...
function resolveScopedClaudeProjectDirs(cwd: string): string[] {
  const direct = claudeProjectDirFromCwd(cwd);
  const fallback = new Set<string>();
  if (fs.existsSync(direct)) fallback.add(direct);
  if (!fs.existsSync(CLAUDE_PROJECTS)) return [...fallback];
  ...
  for (const dir of fs.readdirSync(CLAUDE_PROJECTS, { withFileTypes: true })) {
    ...
    if (sameProject(evt?.cwd, cwd)) fallback.add(dirPath);
  }
  return [...fallback];
}
