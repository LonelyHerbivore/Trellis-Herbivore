...
describe("claudeListSessions / claudeExtractDialogue", () => {
  ...
  it("uses resolved cwd for scoped Claude project dir lookup", () => {
    const scopedCwd = nodePath.resolve("mem-int-project");
    const scopedDir = nodePath.join(
      CLAUDE_PROJECTS,
      process.platform === "win32"
        ? scopedCwd.replace(/[\\/:_]/g, "-")
        : scopedCwd.replace(/[/_]/g, "-"),
    );
    const scopedSessionId = "33333333-3333-3333-3333-333333333333";
    const scopedFile = nodePath.join(scopedDir, `${scopedSessionId}.jsonl`);
    ...
    expect(ids).toContain(scopedSessionId);
  });

  it("falls back to scanning session cwd when scoped dir name does not match cwd encoding", () => {
    const scopedCwd = nodePath.resolve("..", "mem-int-project");
    const mismatchedDir = nodePath.join(
      CLAUDE_PROJECTS,
      process.platform === "win32"
        ? nodePath.resolve("mem-int-project").replace(/[\\/:_]/g, "-")
        : "/tmp/mem-int-project".replace(/[/_]/g, "-"),
    );
    const fallbackSessionId = "44444444-4444-4444-4444-444444444444";
    writeJsonl(nodePath.join(mismatchedDir, `${fallbackSessionId}.jsonl`), [
      {
        type: "user",
        cwd: scopedCwd,
        timestamp: "2026-04-15T10:00:00Z",
        message: { role: "user", content: "fallback scoped session" },
      },
    ]);
    const ids = claudeListSessions(mkFilter({ cwd: scopedCwd })).map(
      (s) => s.id,
    );
    expect(ids).toContain(fallbackSessionId);
  });
