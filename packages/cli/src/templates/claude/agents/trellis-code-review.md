...
- `workflow.selection_status: unselected`, a malformed structured workflow, a missing `task.worktree_path`, a disabled `code-review` gate, or a missing `code-review` run entry blocks this review. Report the task-contract issue and stop.
...
- Return the Markdown report only. The main session writes it to `<task-path>/reports/code-review.md` and updates `workflow.review_gates.runs.code-review` with the result, attempt count, and task-relative report path.
