...
- If the strategy is `subagent + worktree`, read `<task-path>/task.json` before choosing a directory. `task.worktree_path` is the sole worktree path; use it even when it differs from `./.trellis/trellis-worktrees/<task-dir-name>`, do not derive or override it from task Markdown, and do not create, switch, nest, or synchronize a worktree.
- On Claude Code, host `Agent(..., isolation: "worktree")` must not override `task.worktree_path`; report a conflict to the main session instead.
