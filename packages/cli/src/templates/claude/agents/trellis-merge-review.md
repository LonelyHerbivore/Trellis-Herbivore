...
- `workflow.selection_status: unselected`, a malformed structured workflow, a missing `task.worktree_path`, a disabled `merge-review` gate, or a missing `merge-review` run entry blocks this review. Report the task-contract issue and stop.
...
- **If the `<!-- trellis-hook-injected -->` marker is present**: task artifacts are already loaded after the task-record check.
...
- Return the Markdown report only. The main session writes it to `<task-path>/reports/merge-review.md` and updates `workflow.review_gates.runs.merge-review` with the result, attempt count, and task-relative report path.
...
1. `<file>:<line>` - <conflict, completeness, acceptance, or integration evidence>
...
- Conflict markers: Passed / Failed
- Completeness and acceptance: Passed / Failed
