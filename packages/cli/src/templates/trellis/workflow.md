...
Then run `task.py claim-worktree <task-dir>` to validate and persist the one actual path: current-checkout reuses the running checkout, new-worktree creates the default `.trellis/trellis-worktrees/<task-dir-name>` once, and existing-worktree accepts only a registered worktree from the same repository. Use `task.py resolve-worktree <task-dir> --json` to recover the path; never create a nested worktree from an already linked checkout.
[/workflow-state:planning]
...
Sub-agent dispatch protocol applies to all platforms and all sub-agents involved in code development or review: every dispatch prompt starts with `Active task: <task path>` and `Actual worktree: <task.json.worktree_path>` before role-specific instructions.
...
Dispatch prompt starts with `Active task: <task path>` and `Actual worktree: <task.json.worktree_path>`. For class-2 Codex/Copilot/Gemini/Qoder sub-agents, this same two-line prefix is mandatory before role-specific instructions. Read task.json first, verify the second line matches its recorded path, and recover the same path after `fork_turns="none"`; do not use the parent cwd. Then read context: jsonl entries -> `prd.md` -> `design.md if present` -> `implement.md if present`.
...
All code-development and review subagents must read `task.worktree_path` from `<task-path>/task.json` and use that recorded path as the sole working directory; do not derive or replace it from `<task-dir-name>`. Final integration uses `task.py merge-worktree <task-dir>` only after source and target are clean and the target is on `base_branch`; conflicts remain in place for manual `git merge --abort` recovery.
...
- **Agent type**: `trellis-implement`
- **Task description**: Implement the reviewed task artifacts, consulting materials under `{TASK_DIR}/research/`; finish by running project lint and type-check
- **Dispatch prompt guard**: prompt **必须**先写两行 `Active task: <task path>` 和 `Actual worktree: <task.json.worktree_path>`，然后显式说明该子代理已经是 `trellis-implement`，必须直接实现，不能再分派新的 `trellis-implement` / `trellis-check`。
