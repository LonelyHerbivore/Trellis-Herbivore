...
If the chosen strategy is `subagent + worktree`, all code-development subagents must use the same `./.claude/trellis-worktrees/<task-dir-name>` path.
...
- 如果选择 `subagent + worktree`，将 `./.claude/trellis-worktrees/<task-dir-name>` 固定为所有代码开发子代理共享的路径。
