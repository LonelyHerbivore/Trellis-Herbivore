...
- **Active task status `planning` + `prd.md` exists** → stay in Phase 1. Lightweight tasks can be PRD-only; complex tasks need `design.md` + `implement.md`. For the Claude Code path, keep requirement clarification in `prd.md`, continue repository-first follow-up questioning aligned to `D:\Trellis\matt-skills-main\skills\productivity\grill-me`, and only enter development strategy decisions after that clarification has converged. Before `task.py start`, record the decisions in the task documents: development mode, branch vs worktree, default flow vs TDD. Complex tasks should keep that record in `implement.md` together with the review-gate plan: `trellis-spec-review` → `trellis-code-review` → `trellis-code-architecture-review`. If the strategy is `subagent + worktree`, pin `./.claude/worktree`; if the strategy is TDD, record `D:\Trellis\matt-skills-main\skills\engineering\tdd` as the reference flow.
