...
- 生成最终中文验收报告、GitHub Release 草稿、npm 发布摘要和待执行命令清单，统一记录在 `release-draft.md`。
...
| `verify-packed-cli` | PASS，CLI 精确锁定 Core `0.6.0-beta.31` |
...
| `verify-npm --package all` | PASS，只读确认 beta.31 已在 npm latest 可见 |
...
- npm beta.31 已存在且当前 HEAD 不在 `v0.6.0-beta.31`；正式发布需要后续 bump 到 beta.32 并创建匹配 tag。
